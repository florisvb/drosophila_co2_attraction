...
def plot_trajectories(ax, paths, keys, key_lengths=None, downsample=10):
    ...
    for p, path in enumerate(paths):
        ...
        for key in keys[p]: 
            ...
            if 0:
                fpl.colorline(ax, x, y, t, linewidth=0.5, norm=None, zorder=10, alpha=1, linestyle='solid', color='black', axis_size_inches=(1.5,1.5), hack_round_caps=True)
                ...
                fpl.colorline(ax, 480-y, x, t, linewidth=0.5, norm=None, zorder=10, alpha=1, linestyle='solid', color='black', axis_size_inches=(1.5,1.5), hack_round_caps=True)
                ...
                #fpl.colorline(ax, y, x, t, linewidth=0.5, norm=None, zorder=10, alpha=1, linestyle='solid', color='black', axis_size_inches=(1.5,1.5), hack_round_caps=True)
                #ax.plot(y,x,color='black',zorder=10,linewidth=0.5)
                figurefirst.deprecated_regenerate.mpl('plot',
                                 layout, figure, axis, fifidatafile, 'Trajectory' ,
                                ['x position',
                                 'y position'],
                                 y,x,color='black',zorder=10,linewidth=0.5)

                #circle_inner = matplotlib.patches.Circle((240, 384), 75, facecolor='red', edgecolor='none', linestyle='solid', linewidth=0.5, zorder=20, alpha=0.25)
                figurefirst.deprecated_regenerate.mpl_patch('Circle', layout, figure, axis, fifidatafile, 'inner circle', [], 
                                        (240, 384), 75, facecolor='red', edgecolor='none', linestyle='solid', linewidth=0.5, zorder=20, alpha=0.25)
                #circle_outer = matplotlib.patches.Circle((240, 384), 220, facecolor='gray', edgecolor='black', linestyle='solid', linewidth=0.5, zorder=20, alpha=0.25)
                figurefirst.deprecated_regenerate.mpl_patch('Circle', layout, figure, axis, fifidatafile, 'outer circle', [], 
                                        (240, 384), 220, facecolor='gray', edgecolor='black', linestyle='solid', linewidth=0.5, zorder=20, alpha=0.25)
                #ax.add_artist(circle_outer)
                #ax.add_artist(circle_inner)
                #ax.set_ylim(144,624)
                #ax.set_xlim(0,480)
    ...
    figurefirst.deprecated_regenerate.mpl('set_frame_on', ax[0], ax[1], ax[2], fifidatafile, 'set_frame_on', [], 
                              False)
    
    #ax.set_rasterization_zorder(1000)
    figurefirst.deprecated_regenerate.mpl('set_rasterization_zorder', ax[0], ax[1], ax[2], fifidatafile, 'set_rasterization_zorder', [], 
                              1000)
    #ax.set_ylim(25,455)
    
    
def plot_h2o_trajectory(layout):
    ax = [layout, 'trajectories', 'h2o']
...
def plot_eth_trajectory(layout):
    ax = [layout, 'trajectories', 'ethanol']
...
def plot_vinegar_trajectory(layout):
    ax = [layout, 'trajectories', 'vinegar']
...
def plot_ethacet_trajectory(layout):
    ax = [layout, 'trajectories', 'ethyl-acetate']
...
if __name__ == '__main__':
    ...
    plot_all_trajectories_on_figure(version='new')
