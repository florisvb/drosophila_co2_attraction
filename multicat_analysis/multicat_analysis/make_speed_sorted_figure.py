...
def make_cross_directory_figure(directory, label, flowrate=1, use_speed_intercept=None, figure='co2', add_flowrate_to_label=False):
    speed_svg = None

    print label, flowrate, figure
    if 'single_concentration_model' in figure:
        svg = co2_paper_locations.figure_template_locations.figure5_single_concentration_model
        speed_svg = None #co2_paper_locations.figure_template_locations.figure5_walking_arena_genetics_speed_control
    elif 'concentration' in figure:
        svg = co2_paper_locations.figure_template_locations.figure5_single_concentration_model
        speed_svg = None #co2_paper_locations.figure_template_locations.figure5_walking_arena_genetics_speed_control
    elif 'control' in figure and 'co2' in figure:
        ...
        speed_svg = None #co2_paper_locations.figure_template_locations.figure5_walking_arena_genetics_speed
    elif 'ethanol' in figure or 'vinegar' in figure:
        svg = co2_paper_locations.figure_template_locations.supp_fig_8_eth_vinegar
        speed_svg = None
    elif 'ir25a_ir40a' in figure:
    ...
    if '_hot' in label:
        DNC.append('24hrs')
        DNC.append('noon')
    if label in ['ir25a2_BAC', 'ir40a']:
        DNC.append('hot')
    directories = mta.read_hdf5_file_to_pandas.get_filenames(directory, contains='', does_not_contain=DNC)

    if not add_flowrate_to_label:
        pass
    else:
        label = label + '_' + str(flowrate) # for single flies

    if 'control' in figure:
        label = 'control_'+label
...
def make_summary_of_genetic_experiments(figure='co2'):
    ...
    if figure == 'co2' or figure == 'co2-control':
        labels = []
        labels_supp = []
        labels_ir25a = []
        labels = ['gr63a', 'ir64a', 'double_gr63_ir64', 
                  'hcs', 'anosmic', 'orco', 'M37ir25a2', 'antennaless', 'M106', 'ir25a2_BAC', 'ir8aM120', 'ir40a']
    elif 'concentration' in figure:
        labels = ['hcs_00', 'hcs_04', 'hcs_12', 'hcs_35']
        #labels_ir25a = ['ir25a_nothot', 'ir25a_hot', 'M37ir25a2_nothot', 'M37ir25a2_hot', 'ir25a2_BAC_nothot', 'ir25a2_BAC_hot']
        #labels_ir21a_ir93a = ['ir40a_nothot', 'ir40a_hot', 'ir21a_nothot', 'ir21a_hot', 'ir93aM98']
        #labels.extend(labels_supp)
        #labels.extend(labels_ir25a)
        #labels.extend(labels_ir21a_ir93a)
    elif 'ethanol' in figure:
        labels = ['eth_hcs', 'eth_anosmic', 'eth_orco', 'eth_M37ir25a2', 'eth_antennaless', 'eth_M106', 'eth_ir25a2_BAC', 'eth_ir8aM120']#, 'eth_ir93aM98']
        ...
        labels = ['ir25a_nothot', 'ir25a_hot', 'M37ir25a2_nothot', 'M37ir25a2_hot', 'ir25a2_BAC_nothot', 'ir25a2_BAC_hot',  'ir40a_nothot',  'ir40a_hot']
...
def make_colormaps(svg):
    ...
    layout.axes[('cmap_h', 'speed_cmap_h')]._custom([], 'fly_plot_lib.plot.colorbar', colormap='hot', orientation='horizontal')
    layout.axes[('cmap_h', 'pref_ind_cmap_h')]._custom([], 'fly_plot_lib.plot.colorbar', colormap='seismic', orientation='horizontal')
...
def make_hcs_full(concentration_dataset=False, odor='co2', figure='co2'):
    if 'control' in figure and 'co2' in figure:
        svg = co2_paper_locations.figure_template_locations.figure5_walking_arena_genetics_only_CO2_control
    elif 'co2' in figure:
        svg = co2_paper_locations.figure_template_locations.figure5_walking_arena_genetics_only_CO2
    elif 'ethanol' in figure or 'vinegar' in figure:
    ...
    if odor == 'co2':
        ...
        if 'control' in figure:
            label = 'control_'+label
        plot_nflies_vs_speed_scatter.plot_odor_response_speed_and_time_matrix(paths, flowrates=flowrate, flowrate_label=label, 
