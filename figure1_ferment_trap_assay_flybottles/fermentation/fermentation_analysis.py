...
def plot_preference_index(ax, t):
    ...
    fpl.scatter_box(ax, 48, pi_2day, xwidth=16, ywidth=2, color=(0.001,0.001,0.001), edgecolor=(0.001,0.001,0.001), flipxy=False, shading='95conf', markersize=2, linewidth=1, use='mean', optimize_scatter_distance=True)
...
def make_figure():
    ...
    alcohol_volume = np.vstack((alcohol_volume_1, alcohol_volume_2, alcohol_volume_3))
    ...
    r, p = plot_preference_index(layout.axes[('mpl','pref_index')], t)
    x = np.linspace(1*24,13*24)
    y = np.zeros_like(x)
    zero, = layout.axes[('mpl','pref_index')].plot(x,y,linestyle='--',color='black',linewidth=1,zorder=-100)
    dashes = [2,2,2,2]
