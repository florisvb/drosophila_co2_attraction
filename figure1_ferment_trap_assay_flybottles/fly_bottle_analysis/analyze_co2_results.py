...
def plot_calibration_supplemental_figure():
    ...
    plot_calibration_concentrations(layout.axes[('co2_calibration', 'calibration_timecourse')])
    plot_calibration_values(layout.axes[('co2_calibration', 'calibration_curve')])
...
def plot_calibration_concentrations(ax=None):
    if ax is None:
        fig = plt.figure()
    ...
    figurefirst.mpl_functions.adjust_spines(ax, ['left', 'bottom'], xticks=[0, 300, 600], yticks=[0,1000,2000])
    ax.set_xlabel('Time, sec')
    ax.set_ylabel('Measured CO2 concentration, ppm')
...
def plot_calibration_values(ax=None):
    ...
    for bottle in qnoyeast_bottle_files:
        t, f = load_bottle_data_and_align_peaks(bottle)
        ax.vlines( np.max(f), 0, model.get_val(np.max(f)), color='brown' )
        ax.hlines( model.get_val(np.max(f)), 0,  np.max(f), color='brown')
...
def make_plot():
    ...
    bottle_values_noyeast, bottle_values_200mg, bottle_values_400mg, bottle_values_200mg_flies = plot_calibration_values() # add ax if you want to direct the plot
    ...
    line, = layout.axes_groups['bottles']['fly_bottles'].plot(x,y,linestyle='--',color='red',linewidth=1,zorder=-100)
    dashes = [2,2,2,2]
    ...
    figurefirst.mpl_functions.adjust_spines(layout.axes_groups['bottles']['fly_bottles'], ['left', 'bottom'], yticks=yticks, xticks=xticks, spine_locations={'bottom': 5, 'bottom': 5}, linewidth=0.5)
